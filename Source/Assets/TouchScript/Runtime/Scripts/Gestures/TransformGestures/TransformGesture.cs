...
namespace TouchScript.Gestures.TransformGestures
{
    ...
    public class TransformGesture : TwoPointTransformGestureBase
    {
        ...
        {
            get => projection == ProjectionType.Layer ? projectionLayer.WorldProjectionNormal : projectionPlaneNormal;
            set
            {
        ...
        public Vector3 LocalDeltaPosition => TransformUtils.GlobalToLocalVector(cachedTransform, DeltaPosition);
        ...
        private Vector2 _lastNonZeroDelta1 = Vector3.zero;
        private Vector2 _lastNonZeroDelta2 = Vector3.zero;

        public bool MaxScaleReached = false;
        ...
        {
            var newVector = projectionParams.ProjectTo(newScreenPos2, TransformPlane) - projectionParams.ProjectTo(newScreenPos1, TransformPlane);
            var oldVector = projectionParams.ProjectTo(oldScreenPos2, TransformPlane) - projectionParams.ProjectTo(oldScreenPos1, TransformPlane);
        ...
        protected override Vector3 doTwoPointTranslation(Vector2 oldScreenPos1, Vector2 oldScreenPos2, Vector2 newScreenPos1, Vector2 newScreenPos2, float dR, float dS, ProjectionParams projectionParams)
        {
            if (isTransforming)
            {
                ...
                var diff1 = new1 - old1;
                if (diff1 == Vector3.zero)
                {
                    diff1 = _lastNonZeroDelta2 * -1f;
                }
                else
                {
                    _lastNonZeroDelta1 = diff1;
                }

                var new2 = projectionParams.ProjectTo(newScreenPos2, TransformPlane);
                var old2 = projectScaledRotated(oldScreenPos2, dR, dS, projectionParams);
                var diff2 = new2 - old2;
                if (diff2 == Vector3.zero)
                {
                    diff2 = _lastNonZeroDelta1 * -1f;
                }
                else
                {
                    _lastNonZeroDelta2 = diff2;
                }

                if (MaxScaleReached)
                {
                    return CalculateTwoPointTranslation(diff1, diff2);
                }
                else
                {
                    return diff1;
        ...
        /// <returns></returns>
        private Vector3 CalculateTwoPointTranslation(Vector3 vector1, Vector3 vector2)
        {
            // Calculate the dot product
            var dot = Vector3.Dot(vector1.normalized, vector2.normalized);
            // Let's only consider values between -1 (opposite vectors) and 0 (perpendicular or parallels)
            var factor = Mathf.Clamp(dot, -1, 0);
            // Let's find direction
            // returns vector1 for dot > 0
            // the difference between the two vectors for -1 < dot < 0
            var direction = vector1 - (vector2 * factor);
            return direction;
        }
        ...
        public float GetDeltaScale()
        {
            if (NumPointers < 2)
            {
                return 1f;  // as if TransformType.Scaling was disabled
            }
            ...
            return doScaling(oldScreenPos1, oldScreenPos2, newScreenPos1, newScreenPos2, projectionParams);
        }
        ...
        protected override void drawDebug(int pointers)
        {
            ...
            if (!DebugMode) return;
            if (projection is ProjectionType.Global or ProjectionType.Object)
            {
        ...
        private void updateProjectionPlane()
        {
            ...
            switch (projection)
            {
                case ProjectionType.Layer:
                    transformPlane = projectionLayer == null ? new Plane(cachedTransform.TransformDirection(Vector3.forward), cachedTransform.position) : new Plane(projectionLayer.WorldProjectionNormal, cachedTransform.position);
                    break;
