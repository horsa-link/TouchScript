...
namespace TouchScript.Layers.UI
{
    ...
    public class TouchScriptInputModule : BaseInputModule//TODO: bring back to internal sealed?
    {
        ...
        private TouchScriptInputModule()
        {
            ...
            {
                raycastersProp = Type.GetType(Assembly.CreateQualifiedName("UnityEngine.UI", "UnityEngine.EventSystems.RaycasterManager")).GetField("s_Raycasters", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        public Canvas GetCanvasForRaycaster(BaseRaycaster raycaster)
        {
            ...
            if (!raycasterCanvasCache.TryGetValue(id, out var canvas))
            {
        ...
        /// </summary>
        public class UIStandardInputModule
        {
            ...
            /// <param name="go"></param>
            /// <returns></returns>
            private bool IsGoIgnored(GameObject go)
            {
                if (!go) return false;
                if (_ignoreGos.Count <= 0) return false;
                var containsGoDirect = _ignoreGos.Contains(go);
                if (containsGoDirect) return true;
                var containsGoIndirect = false;
                for (var i = 0; i < _ignoreGos.Count; i++)
                {
                    if (!go.transform || !_ignoreGos[i] || !_ignoreGos[i].transform || !go.transform.IsChildOf(_ignoreGos[i].transform)) continue;
                    containsGoIndirect = true;
                    break;
                }

                return containsGoIndirect;
            ...
            {
                _ignoreGos.RemoveAll(go => go == null);
            }
            ...
            /// <param name="go"></param>
            /// <returns></returns>
            public bool AddGoIgnored(GameObject go, bool skipCleanList = false)
            {
                if (!_ignoreGos.Contains(go))
                {
                    _ignoreGos.Add(go);
            ...
            /// <param name="go"></param>
            /// <returns></returns>
            public bool RemoveGoIgnored(GameObject go, bool skipCleanList = false)
            {
                var result = _ignoreGos.Remove(go);
            ...
            private Dictionary<int, PointerEventData> m_PointerData = new(10);

            public bool IsPointerOverGameObject(int pointerId)
            {
                var lastPointer = GetLastPointerEventData(pointerId);
                if (lastPointer != null) return lastPointer.pointerEnter != null;
                return false;
            }
            ...
            public bool GetPointerData(int id, out PointerEventData data, bool create)
            {
                ...
                {
                    data = new PointerEventData(input.eventSystem) {pointerId = id,};
            ...
            protected void DeselectIfSelectionChanged(GameObject currentOverGo, BaseEventData pointerEvent)
            {
                ...
                if (selectHandlerGO != input.eventSystem.currentSelectedGameObject) input.eventSystem.SetSelectedGameObject(null, pointerEvent);
            }
            ...
            {
                if (input.eventSystem.currentSelectedGameObject == null) return false;
            ...
            private bool SendMoveEventToSelectedObject()
            {
                ...
                var allow = Input.GetButtonDown(input.HorizontalAxis) || Input.GetButtonDown(input.VerticalAxis);
                ...
                //ConsoleLogger.Log($"{m_ProcessingEvent.rawType} axis: {m_AllowAxisEvents} value: ({x},{y})");
            ...
            private bool SendSubmitEventToSelectedObject()
            {
                ...
                var data = input.GetBaseEventData();
                if (Input.GetButtonDown(input.SubmitButton)) ExecuteEvents.Execute(input.eventSystem.currentSelectedGameObject, data, ExecuteEvents.submitHandler);

                if (Input.GetButtonDown(input.CancelButton)) ExecuteEvents.Execute(input.eventSystem.currentSelectedGameObject, data, ExecuteEvents.cancelHandler);
                return data.used;
            }
            ...
            public virtual void ProcessUpdated(object sender, PointerEventArgs pointerEventArgs)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    var moving = data.IsPointerMoving();

                    if (moving && data.pointerDrag && !data.dragging && ShouldStartDrag(data.pressPosition, data.position, input.eventSystem.pixelDragThreshold, data.useDragThreshold))
                    {
                    ...
                    if (pointer is MousePointer mousePointer && !Mathf.Approximately(mousePointer.ScrollDelta.sqrMagnitude, 0.0f))
                    {
            ...
            public virtual void ProcessPressed(object sender, PointerEventArgs pointerEventArgs)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (IsGoIgnored(currentOverGo))
                    {
                        data.Use(); // in this way (during the whole pointer lifecycle, except for the "ProcessUpdated" event) we know if the pointer was added but its Go was invalid
                        continue;
                    }
                    ...
                    //ConsoleLogger.Log($"Pressed: {newPressed}");
                    ...
                    if (newPressed == data.lastPress) // ?
                    {
                        ...
                        if (diffTime < 0.3f) ++data.clickCount;
                        else data.clickCount = 1;
            ...
            public virtual void ProcessReleased(object sender, PointerEventArgs pointerEventArgs)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    // Ignore the pointer if its Go has already been ignored by "ProcessPressed" (second one to be called in the frame),
                    // otherwise it could be a "PointerDown" over a valid Go and a "PointerUp" over an invalid Go
                    if (data.used && IsGoIgnored(currentOverGo))
                    {
                        data.Use();
                        continue;
                    }
            ...
            public virtual void ProcessRemoved(object sender, PointerEventArgs pointerEventArgs)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    // Ignore the pointer if its Go has already been ignored by "ProcessPressed" (second one to be called in the frame)
                    if (data.used && IsGoIgnored(over.Target.gameObject))
                    {
                        data.Use();
                        continue;
                    }
