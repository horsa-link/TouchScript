    ...
    public sealed class TouchManagerInstance : DebuggableMonoBehaviour, ITouchManager
    {
...
#if TOUCHSCRIPT_DEBUG
                        if (DebugMode) ConsoleLogger.LogWarning($"Pointer with id [{id}] is requested to MOVE to but no pointer with such id found.");
#endif
                        return;
                    }
                }
        ...
        internal void INTERNAL_PressPointer(int id)
        {
            lock (pointerLock)
            {
...
#if TOUCHSCRIPT_DEBUG
                        if (DebugMode)
                            ConsoleLogger.LogWarning($"Pointer with id [{id}] is requested to PRESS but no pointer with such id found.");
#endif
                        return;
                    }
                }
                ...
                if (!pointersPressed.Add(id))
                    if (DebugMode)
                        ConsoleLogger.LogWarning($"Pointer with id [{id}] is requested to PRESS more than once this frame.");
#else
        ...
        internal void INTERNAL_ReleasePointer(int id)
        {
            lock (pointerLock)
            {
...
#if TOUCHSCRIPT_DEBUG
                        if (DebugMode)
                            ConsoleLogger.LogWarning($"Pointer with id [{id}] is requested to END but no pointer with such id found.");
#endif
                        return;
                    }
                }
                ...
                if (!pointersReleased.Add(id))
                    if (DebugMode)
                        ConsoleLogger.LogWarning($"Pointer with id [{id}] is requested to END more than once this frame.");
#else
        ...
        internal void INTERNAL_RemovePointer(int id)
        {
            lock (pointerLock)
            {
...
#if TOUCHSCRIPT_DEBUG
                        if (DebugMode)
                            ConsoleLogger.LogWarning($"Pointer with id [{id}] is requested to REMOVE but no pointer with such id found.");
#endif
                        return;
                    }
                }
                ...
                if (!pointersRemoved.Add(pointer.Id))
                    if (DebugMode)
                        ConsoleLogger.LogWarning($"Pointer with id [{id}] is requested to REMOVE more than once this frame.");
#else
        ...
        internal void INTERNAL_CancelPointer(int id)
        {
            lock (pointerLock)
            {
...
#if TOUCHSCRIPT_DEBUG
                        if (DebugMode)
                            ConsoleLogger.LogWarning($"Pointer with id [{id}] is requested to CANCEL but no pointer with such id found.");
#endif
                        return;
                    }
                }
                ...
                if (!pointersCancelled.Add(pointer.Id))
                    if (DebugMode)
                        ConsoleLogger.LogWarning($"Pointer with id [{id}]] is requested to CANCEL more than once this frame.");
#else
        ...
        private void createCameraLayer()
        {
            if (layerManager.LayerCount == 0 && ShouldCreateCameraLayer)
            {
                if (Camera.main)
                {
                    if (Application.isEditor) ConsoleLogger.Log("No touch layers found, adding StandardLayer for the main camera. (this message is harmless)");
        ...
        private void createInput()
        {
            ...
            {
                if (Application.isEditor) ConsoleLogger.Log("No input source found, adding StandardInput. (this message is harmless)");
                GameObject obj = null;
...
#if TOUCHSCRIPT_DEBUG
                    if (DebugMode)
                        ConsoleLogger.LogWarning($"Id [{id}] was in UPDATED list but no pointer with such id found.");
#endif
                    continue;
                }
                list.Add(pointer);
...
#if TOUCHSCRIPT_DEBUG
                    if (DebugMode)
                        ConsoleLogger.LogWarning($"Id [{id}] was in PRESSED list but no pointer with such id found.");
#endif
                    continue;
                }
                list.Add(pointer);
        ...
        private void updateReleased(List<int> pointers)
        {
            ...
            for (var i = 0; i < releasedCount; i++)
            {
                ...
                {
#if TOUCHSCRIPT_DEBUG
                    if (DebugMode) ConsoleLogger.LogWarning($"Id [{id}] was in RELEASED list but no pointer with such id found.");
#endif
                    continue;
                }
        ...
        private void updateRemoved(List<int> pointers)
        {
            ...
            for (var i = 0; i < removedCount; i++)
            {
                ...
                {
#if TOUCHSCRIPT_DEBUG
                    if (DebugMode) ConsoleLogger.LogWarning($"Id [{id}] was in REMOVED list but no pointer with such id found.");
#endif
                    continue;
                }
...
#if TOUCHSCRIPT_DEBUG
                    if (DebugMode)
                        ConsoleLogger.LogWarning($"Id [{id}] was in CANCELLED list but no pointer with such id found.");
#endif
                    continue;
                }
                idToPointer.Remove(id);
