/*
 * Adapted from TouchScript.InputSources.InputHandlers.MouseHandler, originally authored by
 * @author Valentin Simonov / http://va.lent.in/
 * @author Jorrit de Vries
 */
...
namespace TouchScript.InputSources.InputHandlers
{
    ...
    /// <summary>
    /// Unity mouse handling implementation which can be embedded and controlled from other (input) classes. Checks if
    /// the mouse is from the set target display.
    /// </summary>
    public class MultiWindowMouseHandler : IMultiWindowInputHandler, IDisposable
    {
        #region Consts
        ...
        public MultiWindowMouseHandler(PointerDelegate addPointer,
                                       PointerDelegate updatePointer,
                                       PointerDelegate pressPointer,
                                       PointerDelegate releasePointer,
                                       PointerDelegate removePointer,
        ...
        {
            var buttons = Pointer.PointerButtonState.Nothing;
        ...
        #region Display Relative Position

        private Vector3 GetDisplayRelativeMouseAt(Vector2 mousePosition)
        {
#if UNITY_EDITOR
            return DisplayRelativeMouseAt(mousePosition);
#else
            return Display.RelativeMouseAt(mousePosition);
#endif
        }
        
#if UNITY_EDITOR
        // See https://forum.unity.com/threads/multi-display-canvases-not-working-in-5-4-2.439429/#post-6100827 for
        // retrieving the display index in the editor
        ...
        static void InitializeDisplayRelativeMouseAt()
        {
            ...
            lastFocusedPlayModeView = playModeViewType.GetField("s_LastFocused", BindingFlags.NonPublic | BindingFlags.Static);
#else
            var playModeViewType = assembly.GetType("UnityEditor.GameView");
            lastFocusedPlayModeView = playModeViewType.GetField("s_LastFocusedGameView", BindingFlags.NonPublic | BindingFlags.Static);
#endif
            targetDisplay = playModeViewType.GetField("m_TargetDisplay", BindingFlags.NonPublic | BindingFlags.Instance);
        }
        
        private Vector3 DisplayRelativeMouseAt(Vector2 pointerPosition)
        {
            Vector3 pos = pointerPosition;

            var displayIndex = 0;

            var playModeView = lastFocusedPlayModeView.GetValue(null);
            if (playModeView != null)
            {
                displayIndex = (int)targetDisplay.GetValue(playModeView);
            }
