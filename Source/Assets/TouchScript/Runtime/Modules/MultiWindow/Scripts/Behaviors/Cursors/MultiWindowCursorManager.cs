...
namespace TouchScript.Behaviors.Cursors
{
    ...
    /// </summary>
    public class MultiWindowCursorManager : MonoBehaviour
    {
        ...
        private Dictionary<int, PointerCursor> cursors = new(10);
        ...
        private void pointersAddedHandler(object sender, PointerEventArgs e)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                // Added for multi display
                if (!IsPointerFromDisplay(pointer)) continue;
        ...
        private bool IsPointerFromDisplay(IPointer pointer)
        {
            var inputSource = pointer.InputSource;
            if (inputSource is IMultiWindowInputHandler multiWindowInputHandler)
            {
                if (multiWindowInputHandler.TargetDisplay != targetDisplay) return false;
            }
